...
export default function Header({ data }) {
    ...
    return (
        <Box component='nav' sx={{ flexGrow: 1 }}>
            <AppBar position="relative" elevation={0} sx={{ bgcolor: '#f8f8f8', color: '#fff'}}>
                <Container maxWidth='lg'>
                    <Toolbar sx={{ p: 0, minHeight: 30 }}>plop</Toolbar>
                </Container>
            </AppBar>
            <AppBar position="static" elevation={0} sx={{ bgcolor: '#2d2d2d'}}>
                <Container maxWidth='lg'>
                    <Toolbar sx={{ p: 0, minHeight: 60 }}>
                        <Link href='/'>
                            <a>
                                <svg
            ...
            <AppBar position="relative" elevation={0} sx={{ bgcolor: '#525050', color: '#fff'}}>
                <Container maxWidth='lg'>
                    <Toolbar sx={{ p: 0, minHeight: 50 }}>
                        <List sx={{ display:'flex', flexDirection:'row', p:0 }}>
                            {data.map( category => (
                                <ListItem key={category.name} sx={{ p:0, pr: 3, textTransform: 'capitalize'}}>
                                    <Link href={`/category/${encodeURIComponent(category.slug)}`} >
                                        <Box component='a' sx={{ fontSize: 16, color: '#fff', cursor: 'pointer'}}>{category.name}</Box>
                                    </Link>
                                </ListItem>
                            ))}
                        </List>
            ...
            <AppBar position="relative" elevation={0} sx={{ bgcolor: '#2d2d2d', color: '#fff', mb:{ xs :2, sm: 6 }}}>
                <Container maxWidth='lg'>
                    <Toolbar sx={{ p: 0, minHeight: 50 }}>
